...
// 定义了func type 需要自己手动实现具体逻辑代码，但是参数和返回值按照定义的
...
func (p *Pool) Close() {
	p.mu.Lock()
	clients := p.clients
	p.clients = nil //1 设置为nil
	p.mu.Unlock()

	if clients == nil {
		return
	}
	close(clients)                // close chan
	for client := range clients { // 再读取chan 做循环close
...
func (p *Pool) Get(ctx context.Context) (*ClientConn, error) {
	...
	var err error
	if wrapper.ClientConn == nil { // ClientConn为nil会重新new一个ClientConn
