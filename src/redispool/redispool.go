...
func NewPool(network, addr, password string, size int) (*RedisPool, error) {
	...
	client_list := make([]*redis.Client, 0, size) // len = 0, cap = size
	for i:= 0; i < size; i++{
		...
		if client != nil {
			client_list = append(client_list, client) // 将client 放入client_list
		}
		// 将client_list 中的redis client，放入 rp.pool 可缓冲的chan中
