...
func sigHandleDemo() {
	...
	sigs1 := []os.Signal{syscall.SIGINT, syscall.SIGQUIT} // SIGSTOP SIGKILL是不会被忽略的信号
...
/*
在调用signal.Notify函数之后，立即试图用for语句从signal接收通道中获取信号值，只要sigRecv的值中存在元素值，for语句机会把它们按顺序地接收并赋给迭代变量
sig。否则，for语句就会阻塞，并等待新的元素值被发送到sigRecv的值中。在sigRecv代表的通道类型值被关闭之后，for语句会立即被退出执行，所以我们不用担心
程序会一直在这里循环往复。
*/
