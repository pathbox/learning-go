...
type mutex struct {
	// Full -> locked, empty -> unlocked  chan 满了表示加锁，空表示解锁
	lock chan struct{}
}
...
// TryLock() 方法可以多次调用，成功返回 true，失败返回false。
// true成功表示 m.lock是空的，没有人使用。往里面加值可以成功。你可以获得这个锁
// false失败表示m.lock是满的，有人占用，往里面传值失败。这个锁已被使用
...
func (b *basicBatcher) Get() ([]interface{}, error) {
	...
	if b.maxTime > 0 {
		timeout = time.After(b.maxTime) // 达到超时时间的时候，会往timeout <- 值
	}
	...
	case <-timeout:
		...
		for {
			if b.lock.TryLock() {
				// we have a lock, try to read from channel first in case, something sunck in
				select {
				case items, ok := <-b.batchChan:
					b.lock.Unlock()
					if !ok {
				...
				// 2) A Put or Flush temporarily has the lock. 谁要锁，写操作需要锁
