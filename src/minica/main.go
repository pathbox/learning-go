...
func makeKey(filename string) (*rsa.PrivateKey, error) {
	key, err := rsa.GenerateKey(rand.Reader, 2048) // generate the private key
	if err != nil {
		return nil, err
	}
	der := x509.MarshalPKCS1PrivateKey(key) // marshal key to []byte
	...
	defer file.Close()
	err = pem.Encode(file, &pem.Block{ // now the private key is storing in file
