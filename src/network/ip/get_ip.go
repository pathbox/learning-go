...
// IPs
func IPs() (intranetIPs, publicIPs []string, err error) {
	intranetIPs = make([]string, 0)
	publicIPs = make([]string, 0)
	var ips []string

	if ips, err = getAllIps(); err != nil {
		return
	}
...
func main() {
	intranetIPs, publicIPs, err := IPs()
	if err != nil {
		panic(err)
	}

	fmt.Println(intranetIPs, publicIPs)
}
