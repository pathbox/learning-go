...
func main() {
	...
	if err != nil {
		logger.Printf("Read path error: ", err.Error())
		return
	}
	...
	if err != nil {
		logger.Printf("Read inputFile error: ", err.Error())
		return
	}
...
Outter:
	for {
		...
		if err != nil {
			logger.Printf("Split inputFile error: ", err.Error())
			continue
		}
		...
		for cnt := 0; cnt < LIMIT; cnt++ {
			...
			if err != nil {
				logger.Printf("Split inputFile writting error: ", err.Error())
				return
			}
		}
		piece++
