...
type List struct{ // List 中有Element， Element中有List，互相引用对方
  elems []*Element
  num int
...
func NewSize(size int) *List {  // 根据size大小，默认为1024 初始化一个没有value，但已有空间的list
  if size < defaultSize {
    size = defaultSize
  }
  ...
  for i := range l.elems{
    ...
    l.elems[i] = e //再把这个新建的element 存入l中
  }
...
func (l *List) remove(e *Element) *Element{
  ...
  l.elems[prev].next = next // 被删除元素的prev元素的next 为现在的next
  l.elems[next].prev = prev  // 被删除元素的next元素的prev为 被删除元素的prev

  e.next = -1
  e.prev = -1

  l.num--
  return e // 这里的删除并没有真正释放占用的空间
}
