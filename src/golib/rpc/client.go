...
func (c *Client) Close() error {
	...
	for {
		if c.conns.Len() > 0 {
			...
			co := v.Value.(*conn) // list中存的是一个conn， 现在把这个conn取出来然后close
			co.Close()
			c.conns.Remove(v) // 将这个conn从list中移除
		} else {
			break
		}
	}
...
// rpc 原理解析
// 客户端传方面名和参数，通过网络连接传输到服务端，服务器解析方法名和序列化参数，通过反射或动态语言的方式，调用该方法，传入参数。进行执行该方法的操作
// 如果有返回值，则返回给客户端
